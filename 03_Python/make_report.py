...
Pages
1) Title + KPIs + Monthly Revenue (Python hero chart)
2) KPI Visuals (Python): Monthly Revenue + Top Products + RFM Segments (all on one page)
...
Images (Python-generated by ecommerce__analysis.ipynb):
- 03_Analysis/figures/monthly_revenue.png
- 03_Analysis/figures/top_products.png
- 03_Analysis/figures/rfm_segments.png
...
# Tableau RFM image (keep this one only)
IMG_DIR = BASE_DIR / "05_Tableau" / "exports"
IMG_RFM_TABLEAU = IMG_DIR / "customer_segments.png"

# Python-generated KPI figures (from ecommerce__analysis.ipynb)
IMG_MREV_PY = BASE_DIR / "03_Analysis" / "figures" / "monthly_revenue.png"
IMG_TOP_PY  = BASE_DIR / "03_Analysis" / "figures" / "top_products.png"
IMG_RFM_PY  = BASE_DIR / "03_Analysis" / "figures" / "rfm_segments.png"
...
try:
    ...
    # Links (kept for PDF path reference only)
...
# === Page 1 — KPIs + Hero chart ===
Story.append(Paragraph("E-Commerce & Finance Insights Report", styles["CyanTitle"]))
Story.append(Paragraph("Generated via Python ReportLab • Author: Huzeif Khan", styles["BodyGrey"]))
...
# Hero chart: Monthly Revenue (Python)
if IMG_MREV_PY.exists():
    Story.append(Paragraph("Monthly Revenue Trend", styles["Heading3Cyan"]))
    Story.append(fit_image_keep_ratio(IMG_MREV_PY, max_w=16.5*cm, max_h=8.0*cm))
else:
    Story.append(Paragraph("Monthly Revenue chart not found (expected 03_Analysis/figures/monthly_revenue.png).",
                           styles["SmallGrey"]))

# === Page 2 — Three KPI charts on one page (Python) ===
...
if IMG_MREV_PY.exists():
    ...
    Story.append(fit_image_keep_ratio(IMG_MREV_PY, max_w=16.5*cm, max_h=6.0*cm))
    Story.append(Spacer(1, 6))

# 2) Top Products
if IMG_TOP_PY.exists():
    Story.append(Paragraph("Top 10 Products by Revenue", styles["Heading3Cyan"]))
    Story.append(fit_image_keep_ratio(IMG_TOP_PY, max_w=16.5*cm, max_h=6.0*cm))
    Story.append(Spacer(1, 6))
else:
    Story.append(Paragraph("Top Products chart not found (expected 03_Analysis/figures/top_products.png).",
                           styles["SmallGrey"]))

# 3) RFM Segments (Python pie)
if IMG_RFM_PY.exists():
    Story.append(Paragraph("Customer Segmentation (RFM) — Python", styles["Heading3Cyan"]))
    Story.append(fit_image_keep_ratio(IMG_RFM_PY, max_w=16.5*cm, max_h=6.0*cm))
else:
    Story.append(Paragraph("RFM segments chart not found (expected 03_Analysis/figures/rfm_segments.png).",
                           styles["SmallGrey"]))

# === Page 3 — Tableau RFM image (kept) ===
Story.append(PageBreak())
Story.append(Paragraph("Customer Segmentation (RFM Model)", styles["Heading2Cyan"]))
Story.append(Spacer(1, 6))
if IMG_RFM_TABLEAU.exists():
    Story.append(fit_image_keep_ratio(IMG_RFM_TABLEAU, max_w=16.5*cm, max_h=17*cm))
else:
